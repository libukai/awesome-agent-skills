...
经过半年的发展和迭代，Skill 标准已得到 AI 行业的广泛支持，也成为了类 Claude Code 生态和类 OpenClaw 生态中增强个性化 AI 能力的标准方案。
...
建议安装 `clawhub` 命令行工具来安装和管理技能，安装方式和具体参数详见 [openclaw/clawhub](https://www.npmjs.com/package/clawhub)。

或者也可以使用 `npx clawhub@latest clawhub install <skill-name>` 的方式来直接使用最新版本的工具。

```bash
clawhub search "postgres backups"  # 搜索相关技能
clawhub install <skill-name> # 安装指定名称的技能
clawhub update --all # 升级技能
clawhub update --all --no-input --force # 强制升级技能
```
